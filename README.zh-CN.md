...
截至公开版 **v0.1.8（9 月 20 日）**：支持简体中文、繁体中文和英文；本轮 `v0.1.9` 本地候选的 **34/34 组自动测试通过**。可见 Windows 鼠标键盘验收仍按真实环境单独记录，自动测试与真人界面验收分开说明。[查看维护方式](CONTRIBUTING.md#maintenance-ownership-and-evidence)。
...
截图可以点“粘贴截图”，也可以在截图区域按 Ctrl+V 或拖入一张本机图片；需要换图时可单独移除，文字描述不会被清空。输入页按 `Ctrl+Enter` 就能开始检查，普通 Enter 仍然换行；描述接近 1200 字时会提醒，过长粘贴会保留原文字并说明原因。结果页的“再检查一次”会沿用当前输入，“返回补充说明”会保留截图和描述但作废上一轮报告，“换个问题（清空）”才会清除它们。等待太久时可按“停止等待”，输入仍会保留；右上角选过的语言会在下次打开时记住。
...
- 复核页展示的文字就是“只复制反馈材料”实际复制的文字；保存和打开公开页面是次要选择，不会自动提交
- 复制或打开链接的结果会显示在独立的操作提示位置，不会覆盖诊断说明；切换语言时会保留正在进行的检查阶段和已用时间
...
当前公开版 v0.1.8 尚未签名，Windows 可能显示来源提醒；本轮 v0.1.9 仅为本地候选，未公开发布。运行前请核对发布页上的 SHA-256。不要为了绕过提醒而关闭 Windows 安全保护。
