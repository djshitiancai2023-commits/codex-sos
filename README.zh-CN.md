...
截一张 Codex 错误截图，或写一句发生了什么。Codex SOS 会自动补齐必要的检查信息，整理成**经过隐私复核、方便继续求助的材料**，并给出保守的下一步建议；同时帮助避免重复报错，或把其他软件的问题误投给 Codex。
...
[直接下载便携压缩包](https://github.com/djshitiancai2023-commits/codex-sos/releases/download/v0.1.6/Codex-SOS-0.1.6-win-x64-portable.zip)
...
| 你遇到的情况 | SOS 帮你少做什么 |
|---|---|
| 只知道“Codex 卡住了”，不会找资料 | 自动补齐必要检查，再复制到已有的客服对话中 |
| 官方体检正常，但刚才确实出错了 | 保留你看到的现象，不把绿灯当成“问题已经解决” |
| 官方反馈上传失败，或问题来自其他软件 | 不冒充发送成功，也不把明显无关的问题投给 Codex |

[不用安装，先看三个可核验示例](docs/EXAMPLES_AND_VERIFICATION.md)。示例来自公开的虚构测试资料，不是外部用户实测，也不是本轮新录制的界面截图。

截至 **v0.1.6（9 月 4 日）**：支持简体中文、繁体中文和英文；[公开 Windows 发布记录](https://github.com/djshitiancai2023-commits/codex-sos/actions/runs/33837154773)显示 **26/26 组自动测试通过**。自动测试与真人界面验收分开说明。[查看维护方式](CONTRIBUTING.md#maintenance-ownership-and-evidence)。
...
- 用户确认隐私预览后，推荐先只复制官方需要的栏目材料，不打开网页、不需要登录；需要公开时再用另一个按钮打开 Codex App 问题页
- 复制出的材料还会整理事故时间和时区、屏幕上是否有错误文字、可选的私下 Feedback ID 提示，以及影响范围和已有记录提醒；不增加一张表，也不会要求重现故障
- 看到“feedback upload failed”时会搜索相似公开报告；反馈编号没有变化不会被当成已经发送成功
...
- 用户主动点“只复制反馈材料”时只写入本机剪贴板，不打开 GitHub；点“复制并打开公开问题页”时，网址里也不带报告内容，SOS 不会自动粘贴、登录、点赞或提交。
- 如果现有线索更像其他程序的问题，不会搜索 Codex 官方问题，也不会显示官方反馈按钮。
...
v0.1.0 到 v0.1.6 没有签名；只有安装包和便携版主程序都通过 Windows
验证后，后续版本才会写成“已签名”。签名不能保证 Windows 永远不显示
SmartScreen 提醒。

## 下载与运行

v0.1.6 已通过公开仓库发布。普通 Windows 用户先下载[便携压缩包](https://github.com/djshitiancai2023-commits/codex-sos/releases/download/v0.1.6/Codex-SOS-0.1.6-win-x64-portable.zip)。直接下载该压缩包即可。下载后右键选择“全部解压”，再双击 CodexSOS.exe。下载和使用不需要登录 GitHub，也不需要点赞。

如果你需要开始菜单入口和卸载功能，可选安装程序 `Codex-SOS-Setup-0.1.6.exe`。安装包会自带运行所需组件，用户不需要安装 .NET、Python、OCR 或开发工具。

程序默认显示简体中文，右上角可以随时切换为繁體中文或 English；首页、检查过程、结果和导出材料会一起切换。

发布页也提供 `SHA256SUMS.txt`，可用于核对文件是否完整。

当前 v0.1.6 尚未签名，Windows 可能显示来源提醒。运行前请核对发布页上的 SHA-256。不要为了绕过提醒而关闭 Windows 安全保护。
